...
for word in range(0,words_length):
...
def word_mixer(words_list):
    ...
    while len(words_list) > 5:
        val_fifth = words_list.pop(-5)
        new_words.append(val_fifth)
        val_first = words_list.pop(0)
        new_words.append(val_first)
        val_last = words_list.pop()
        new_words.append(val_last)
    return new_words

print_list = word_mixer(words_list)
for i in range(0,len(print_list),3):
    print(" ".join(print_list[i:i+3]))
